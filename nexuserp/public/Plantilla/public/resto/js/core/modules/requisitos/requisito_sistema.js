function asignar_sistema(checked) {
/////////////toma los checbox marcados y los pone en un arreglo
    ...
    for(var i = 0; i < items.length; i++){
        ...
        arrSistemas.push(items[i].getAttribute('id'));
    }
  //  return arrSistemas;
 	strSistemas = JSON.stringify(arrSistemas);
//   console.log(strSistemas);
	...
	if (requisito.value !== "") {
		...
		http.append("request", "asignar_sistemas");
		http.append("requisito", requisito.value);
		http.append("sistemas", strSistemas);
		var request = new XMLHttpRequest();
		request.open("POST", "ajax_fns_requisito_sistema.php");
		request.send(http);
		request.onreadystatechange = function () {
			...
			if (request.status === 200) {
				...
				swal("Excelente!", resultado.message, "success").then((value) => {
					window.location.href = "FRMrequisito_sistema.php";
				});
			}
		};
	} else {
...
function asignar_requisito_sistema(codigo){
	window.location.href = "FRMasignar_requisito_sistema.php?requisito=" + codigo;
}

function sistema_checkbox(codigo){
	contenedor = document.getElementById("result_sistema");
	requisito = document.getElementById("requisito");
	loadingCogs(contenedor);
	/////////// POST /////////
	var http = new FormData();
	http.append("request","tabla_sistema");
	...
	request.onreadystatechange = function(){
		...
		if(request.status === 200){
			...
			// console.log(document.getElementById('3').checked);
			$('#tabla').DataTable({
				pageLength: 50,
				responsive: true,
				...
				buttons: [
					{extend: 'copy'},
					{extend: 'csv'},
					{extend: 'excel', title: 'Tabla de Sistemas'},
					{extend: 'pdf', title: 'Tabla de Sistemas'},
					{extend: 'print',
	...
	swal({
		...
		switch (value) {
			...
			/*	checkboxes = document.getElementsByClassName("custom-checkbox");
				inputs_boxes = checkboxes.getElementsByTagName("input");
				inputs_boxes="checked";*/
				break;
			default:
			return;
		}
...
function printTable() {
	...
	request.onreadystatechange = function () {
		...
		if (request.status === 200) {
			...
			//tabla
			var data = resultado.tabla;
			contenedor.innerHTML = data;  //Aquí se pinta la tabla en result
			$('#tabla').DataTable({
				pageLength: 50,
				responsive: true,
				dom: '<"html5buttons"B>lTfgitp',
