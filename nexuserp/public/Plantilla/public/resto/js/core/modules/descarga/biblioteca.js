...
function cambioSituacion(codigo, situacion) {
    ...
    request.onreadystatechange = function () {
        ...
        if (request.status === 200) {
            ...
            switch (situacion) {
                case 10:
                    swal("Ok!", "Registro marcado como obsoleto...", "success").then((value) => {
                        console.log(value);
                        window.location.reload();
                    });
                    break;
                case 3:
                    swal("Excelente", "Documento de Bilblioteca Aprobado!!!", "success").then((value) => {
                        console.log(value);
                        window.location.reload();
                    });
                    break;
                case 2:
                    swal("Excelente", "Solcitud registrada satisfactoriamente...", "success").then((value) => {
                        console.log(value);
                        window.location.reload();
                    });
                    break;
                case 0:
                    swal("OK", "Registro eliminado....", "success").then((value) => {
                        console.log(value);
                        window.location.reload();
                    });
                default:
...
function borrarArchivo(codigo) {
    ...
    request.onreadystatechange = function () {
        ...
        if (request.status === 200) {
            ...
            swal("Excelente!", "Registro eliminado satisfactoriamente!!!", "success").then((value) => {
                console.log(value);
                window.location.reload();
            });
        }
...
///////// DESCARGAS EN MASA
async function descargaMultiple() {
    let boton = document.getElementById("btn-masa");
    ...
    if (filas > 0) {
        ...
        for (let i = 1; i <= filas; i++) {
            ...
            if (chk.checked) {
                let archivo = {};
                if (document.getElementById("codigo" + i)) {
                    archivo.codigo = document.getElementById("codigo" + i).value
                }
                if (document.getElementById("cliente" + i)) {
                    archivo.cliente = document.getElementById("cliente" + i).value
                }
                if (document.getElementById("nombre_archivo" + i)) {
                    archivo.nombre_archivo = document.getElementById("nombre_archivo" + i).value
                }
        ...
        swal("Ohoo!", "No hay filas para descargar...", "warning").then((value) => {
            //console.log( value );
            deloadingBtn(boton, '<i class="fas fa-cloud-download-alt"></i> Descargar');
        });
        return;
    }

    //--
    //console.log( document.getElementById("nombre_archivo" + 1).value);
    //console.log( fileList.length, contador );

    if (contador > 0) {
        //console.log( fileList );
        //Inicicaliza el progressbar
        let resultadoDescargas = await descargarArchivosVarios(fileList);
        
       
        console.log(resultadoDescargas);
        if (resultadoDescargas.status) {
            swal("Excelente!", "Archivos descargados satisfactoriamente!!!", "success").then((value) => {
                //console.log( value );
                window.location.reload();
            });
        } else {
            swal("Error", "Es posible que uno o m\u00E1s archivos no se descargaron por alg\u00FAn error en el traslado...", "error").then((value) => {
                //console.log( value );
                deloadingBtn(boton, '<i class="fas fa-cloud-download-alt"></i> Descargar');
            });
        }
    } else {
        swal("Ohoo!", "No hay archivos seleccionados para descargar...", "error").then((value) => {
            //console.log( value );
            deloadingBtn(boton, '<i class="fas fa-cloud-download-alt"></i> Descargar');
        });
    }
...
/////////////////////////
async function descargarArchivosVarios(fileList) {
    tipo = document.getElementById("tipo");
    let vuelta = 0;
    let devuelve = {};
    let arrpromises = [];
    //--
    await asyncForEach(fileList, async (listado) => {
        //console.log(listado);
        arrpromises[vuelta] = await new Promise((resolve, reject) => {
            let cliente = listado.cliente;
            let codigo = listado.codigo;
            let nombre_archivo = listado.nombre_archivo;
            /////////// POST /////////
            let httpPost = new FormData();
            httpPost.append("request", "descargar_documento");
            httpPost.append("categoria", cliente);
            httpPost.append("codigo", codigo);
            httpPost.append("stringArchivo", nombre_archivo);
            let requestPost = new XMLHttpRequest();
            requestPost.open("POST", "ajax_fns_biblioteca.php");
            requestPost.onload = () => {
                if (requestPost.status >= 200 && requestPost.status < 300) {
                  //  console.log(requestPost.responseText);
                    devuelve = JSON.parse(requestPost.response);
                    if (devuelve.status === true) {
                        window.open("download.php?codigo=" + codigo + "&nombre=" + nombre_archivo);
            ...
            requestPost.onerror = () => reject(requestPost.statusText);
            requestPost.send(httpPost);
        });
        vuelta++;
    });

    /// Resolucion de las cargas...
    let retorno = {};
    //console.log( arrpromises );
    await Promise.all(arrpromises).then(values => {
        //console.log(values);
        retorno.status = true;
        retorno.message = 'Archivos descargados satisfactoriamente...';
    }, reason => {
        //console.log(reason);
        retorno.status = false;
        retorno.message = 'Una o varios archivos no han podido ser descargados,...';
    });
