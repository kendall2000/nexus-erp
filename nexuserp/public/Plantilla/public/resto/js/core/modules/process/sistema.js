//funciones javascript y validaciones
	...
	function printTable(codigo){
		...
		request.open("POST", "ajax_fns_sistema.php");
		request.send(http);
	...
	function seleccionarSistema(codigo){
		contenedor = document.getElementById("result");
		loadingCogs(contenedor);
		/////////// POST /////////
		var http = new FormData();
		...
		request.onreadystatechange = function(){
			...
			if(request.status === 200){
				...
				document.getElementById("politica").value = data.politica;
				// Seteo de Select2
				setSelect2("usuario",data.usuario);
	...
	function Grabar(){
		...
		if(nombre.value !== "" && color.value !== "" && usuario.value !== ""){
			/////////// POST /////////
			...
			http.append("politica", politica.value);
			var request = new XMLHttpRequest();
			request.open("POST", "ajax_fns_sistema.php");
			request.send(http);
			request.onreadystatechange = function(){
	...
	function deshabilitarSistema(codigo){
		swal({
			text: "\u00BFDesea quitar a este sistema del listado?, no prodr\u00E1 ser usada despu\u00E9s...",
			icon: "warning",
			buttons: {
				cancel: "Cancelar",
				ok: { text: "Aceptar", value: true },
