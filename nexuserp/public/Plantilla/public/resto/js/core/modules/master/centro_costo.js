//funciones javascript y validaciones
	...
	function printTable(codigo){
		...
		request.open("POST", "ajax_fns_cc.php");
		request.send(http);
			...
			if(request.status === 200){
				...
				$('#tabla').DataTable({
					...
					buttons: [
						{extend: 'copy'},
						{extend: 'csv'},
						{extend: 'excel', title: 'Tabla de Centros de Costos'},
						{extend: 'pdf', title: 'Tabla de Centros de Costos'},
						{extend: 'print',
							...
							}, title: 'Tabla de Centros de Costos'
						}
					]
				});
			}
	...
	function seleccionarCentroCosto(codigo){
		contenedor = document.getElementById("result");
		loadingCogs(contenedor);
		/////////// POST /////////
		var http = new FormData();
	...
	function deshabilitarCentroCosto(codigo){
		swal({
			title: "\u00BFEsta Seguro?",
