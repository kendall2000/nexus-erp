...
function printTable() {
	...
	http.append("request", "tabla_necesidad");
...
function Grabar() {
	...
	lista = document.getElementById('duallistbox1[]');
	...
	if (tipo.value !== "" && nombre.value !== "" && descripcion.value !== "") {
		...
		loadingBtn(boton);
		var http = new FormData();
		http.append("request", "grabar_necesidad");
...
function Modificar() {
	...
	if (codigo.value !== "" && tipo.value !== "" && nombre.value !== "" && descripcion.value !== "") {
		...
		loadingBtn(boton);
		var http = new FormData();
		http.append("request", "modificar_necesidad");
